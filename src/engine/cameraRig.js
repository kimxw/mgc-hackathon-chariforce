...
export function fitView() {
  ...
  // On phone-width viewports, nudge in slightly closer than the strict
  // "fit everything inside every panel's padding" contain fit — that fit
  // is already fairly tight, so this is a small deliberate push, not a
  // rescue (0.5 here previously cropped the chair off-frame — needs to
  // stay modest). panX/panY are already fixed absolute world-space
  ...
  if (innerWidth < 760) { cam.radius *= 0.85; updCam(); }
}
